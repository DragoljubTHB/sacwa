...
Ziel: Anwendung
Wichtig ist als Software Architekt, Entickle oder ProjMan die Specific properties von Cloud Angebot zu verstehen und entsprechend die Anwendung zusammen bauen. 

Ziel: Software as a Service
Cloud Service Model
...
\section{SOA}
Service Orientierte Architektur SOA unterlegt die Annahme dass, ein System aus mehreren kleinen, austauschbaren und entkoppelten Diensten bestehen kann. 

SOA\cite{cloudEssentials} is a flexible set of design principles and standards used for systems
...
domains
Cloud computing is a service delivery model in which shared
services and resources are consumed by the users across the Internet
just like a public utility on an on-demand basis


SOA enables mutual data exchange between programs of different ven-
dors without the need for additional programming or changes to the ser-
...
1. Reuse of services: Various services can be reused by different applica-
...
ability to change the business processes quickly when needed to sup-
...
Wir werden als SOA Technologie REST: REpresentational State Transfer (REST) is a stateless architecture that runs over HTTP. It is used for effective interactions between
clients and services. Benutzen.


Microservices und Serverless versuchen die Komplexität der SOA anzusprechen. Beide Ansätze zwingen auf Separation of Concerns, häufige Deployments und Heterogeneous DSL Domain Specific Language selection.
...
and architectures have to offer\cite{sbarski2017serverless}

\chapter{nichtfunktionalen/technischen Anforderungen zur Entwicklung des Serverless-Ansatzes}
Gegenüberstellung 
Skalliebarkeit ok
Vendor-lockin
Customisation
...
Die Unterschiede zwischen den\cite{king2016aws}
...
\section{Use Cases}
next\cite{sbarski2017serverless}
...
compute as a glue pipelines built to carry out workflows <-> Async Messag Primer\cite{homer2014cloud}

Pipes and Filters\cite{homer2014cloud} <-> lambda event driven
