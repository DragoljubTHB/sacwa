...
Viele traditionelle Software bietet seine \acrshort{SaaS} nicht an. Dies Impliziert laut Chandrasekaran, K. dass dieses Serviceliefermodell sich für Anwendungen wegen Folgendem nicht gut ein eignet:
\newacronym{SLA}{SLA}{Service Level Agreement}
\begin{itemize}
	\item Geringe Latenz kann durch die Entwerfung der gespeicherten Daten für Echtzeit Anwendungen nicht gewährleistet.
	\item Daten Sicherheit kann nicht garantiert werden, weil es nicht sichergestellt werden kann, dass die mitbeteiligte Drittanbieter bei \acrshort{SaaS} die \acrfull{SLA} von Kunden erfühlen.
	\item Software vor Ort kann angefordert sein.
\end{itemize}
...
Die Implementierung einer solchen Funktion geschieht durch die Auswahl der Programmiersprache und der von dem Cloud-Anbieter vorgegebenen Funktionsfassade. Die auszuführende Datei muss alle ihre Abhängigkeiten enthalten.
...
\begin{itemize}
	\item Entwickler Rootzugrifsrechte auf alle Ressourcen eines Servers braucht.
	\item die Priorisierung von Betriebssystemattributen wie CPU, GPU, Networking oder Speichergeschwindigkeit angefordert sind.
...
\begin{itemize}
	...
	\item ein Scriptbehälter, für z.B Cron Aufgaben, benötigt wird. Die Zugriffsrechte sind enger, Fehler sind einfacher zu erkennen und an einer Stelle aggregiert ( siehe CloudWatch ), Deployments können einfacher getriggert werden.
	\item die Entlastung eines Servers von ressourcenintensiven Verarbeitungen dessen Skalierung vermeiden kann.\ref{sec:pipes-filters}
	\item Services vorgibt, die selten benutzt werden.
	\item das Deployment von  \acrshort{API}-Server nicht Verwaltet
