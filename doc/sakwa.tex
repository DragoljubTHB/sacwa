...
Dieses Vorgehen bei der Authentisierung ( siehe \ref{pic:federated-identity} ) basiert auf dem Vertrauen zwischen Cloudanbietern. Der Nutzer kann für mehrere Anwendungen die gleichen Zugangsdaten benutzen. Die Anmeldung erfolgt über den Identitätsanbieter ( wie z.B Google oder Github ). 
...
Die Ergebnisse zeigen zunächst, wie sich die Lambdalaufzeitumgebung initiiert ( siehe \ref{lambda-cold-warm} ). Da die Laufzeitumgebung für jede Anfrage eine neue Lambdafunktion instantiiert, muss der Server neu gestartet werden. Hierzu beweist die Grafik, dass \acrshort{IaaS} und \acrshort{PaaS} besser für längerfristige Prozesse als Lambda geeignet sind ( siehe \ref{lambda-yes-no} ). 
