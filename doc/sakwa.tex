...
\paragraph{Die Reaktion auf Events}\label{par:event-reaction} innerhalb eines verteilten Systems ließ sich mit der vorhandenen Technologien wie Virtualisierte oder Containerised Servern lösen, in dem neue Instanzen zur Verarbeitung von einer Kette von variable Events erzeugt und danach gelöscht. Was zurück auf die oben genannten Aufgaben\ref{sec:iaas} führte.

\paragraph{Das Polling Problem}\label{par:polling}.
Polling ist der Ausdruck für eine zyklische Abfrage über einen Status z.B. von Ports oder Locks über Ressourcen. Die Effizienz bei der Verwendung von Systemressourcen kann geringer als bei Alternativansätze wie z.B Push- oder Pull- Eventmodell.
...
Für die Skalierung eines Softwarekomponenten ist von Vorteil dass Zustandslos ist, eben wie die Funktionale Programmierung. 
...
\label{sec:faas}
\acrshort{FaaS} kann als ein Rechen-Service beschrieben werden, dass nach Anfrage isoliert, unabhängig und granular ausgeführt wird. Dessen \glqq Unit of Deployment\grqq\ und Skalierung ist eine Funktion. 
