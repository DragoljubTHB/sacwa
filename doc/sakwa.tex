...
%WARUUUUUM  Was I,P,S aaS nicht geboten haben so dass FaaS noch entsteht. 
% Hardware anforderung <- Ignoriert
...
\section{kurzer Überblick über Serverless-Angebote/Architekturen}

\subparagraph{Die Architekturentwurfsmuster} helfen uns zu kommunizieren welches Zweck unsere Software erreichen möchte und bieten generische Lösungen für wiederkehrende Probleme bei der Softwareentwicklung.
...
\subsubsection{compute as a backend}
...
\subsubsection{legacy api proxy}
\begin{figure}[H]
...
\subsection{Patterns Serverless}
...
\begin{figure}[H]
	\includegraphics[scale=0.36]{./pics/pattern-priority-queue.png}
\end{figure}

\subsubsection{Federated Identity}
\begin{figure}[H]
	\includegraphics[scale=0.36]{./pics/pattern-federated-identity.png}
\end{figure}


\subsubsection{Fan Out\cite{serverlessArchAWS}}
\label{sec:fan-out}
\begin{figure}[H]
...
\subparagraph{Prinzipien}\label{par:serverless-principles} von Serverless Architeturen: \cite{serverlessArchAWS}
...
\end{wrapfigure}
\paragraph{\acrfull{SQS}}\label{par:sqs} ist eine message queue. Erlaubt die Interaktion von mehreren Publishers und Consumers in eine SQS und verwaltet automatisch das Lebenszyklus der Nachrichten. 
...
\begin{figure}[H]
	\includegraphics[scale=0.2]{./pics/aws/pattern-master-subordinate-data-replication.png}
\end{figure}

\subparagraph{cache shared }
\begin{figure}[H]
	\includegraphics[scale=0.2]{./pics/aws/pattern-cache-shared.png}
\end{figure}
...
\begin{figure}[H]
	\includegraphics[scale=0.2]{./pics/aws/pattern-waterfall.png}
\end{figure}
