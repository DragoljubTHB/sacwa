...
Da die \acrshort{PaaS} und \acrshort{IaaS} pro Stunde abgerechnet werden, \acrshort{FaaS} hingegen pro Anfrage, ergibt sich daraus, dass der Preis und die Performanz je Anfrage, invers proportional zu ihrer Anzahl ist.
...
Es besteht ein Lock-in Risiko, wenn die Portierung von Quellcode auf unterschiedliche Cloudanbieter nicht im Voraus berücksichtigt wurde.
...
% revise
Je spezifischer die Anforderungen sind, desto mehr Eigenschaften der zugrundeliegenden Technologie zu beherrschen ist. Z.B. eine Erweiterung der Funktionalität einer bestimmten Datenbanken durch Lambda, um Konsistente Lese- und Schreiboperationen zu gewährleisten.
...
Webanwendungen von Fehlertoleranz profitieren.
...
Der Bedarf an Werkzeuge für die Entwicklung, den Einsatz, das Testen usw. bleibt, wie in \ref{devops-critic} erläutert wurde,  auch mit dem Serverless Ansatz unberuht.


%\paragraph{Zukunftsperspektiven und Trade-Offs}
%Ontologien habe potenzial dieses Falles hinaus 
Der Begriff \glqq Edge-Computing\grqq\ bezeichnet die Verlagerung von Rechenleistung, Anwendungen, Daten und Services an die Endpunkte eines Netzwerkes. Diese Verlagerung wurde bei der Umsetzung von \acrshort{KOMA} in \autoref{sec:spa} in Form einer \acrshort{SPA} implementiert. Die Vereinigung von Edge- mit Cloud-Computing durch die Verwendung von \acrshort{FaaS} kann einen neuen Potenzial für die Entwicklung von IoT darstellen.
...
Hierzu hilf es zu differenzieren zwischen einer zeitlichen und einer technologischen Flexibilität. 

Die Zeitliche kann dadurch erreicht werden, in dem die Kernfunktionalität einer Anwendung in \acrshort{FaaS} Dienste kapselt, um einen konzeptionellen Beweis durchzuführen. 

Die Technologische durch die oben vorgestellten Architekturendwurfmuster ( siehe \autoref{chap:aws-serverless} ) . Da die unter \acrshort{EDA} integrierte Dienste von einander stark entkoppelt sind, lassen sie sich einfach aggregieren.
