...
Mit der bisher beschriebenen Implementierung der Webanwendung können Daten abgefragt und verarbeitet werden. Diese Ergebnisse stellen ein Feedback dar, womit Konzepte (  siehe \autoref{aspect:proofConcept} ) bewiesen, ausgewertet und weiterentwickelt werden können. % Weiter zur Auswertung Time to market etc.
...
Latenz verkürzt sich bei der Zuteilung einer Replik ( siehe  \autoref{pic:master-slave} ) in der physische Nähe des Consumers.
...
Die SPA besteht aus einem einzigen HTML Dokument. Dadurch vereinfacht man die Konfiguration der Authentifizierung und unterbricht den Fluss der UI-Darstellung zwischen Seiten nicht.
...
Da der Zugriff auf die Datenspeicherung gesichert werden soll, wird die Login-Funktionalität hinzugefügt. In \autoref{pic:koma-components} kann eine Übersicht der nötigen Schritten zur Anmeldung bei AuthO, Authentisierung und Authorisierung erhalten.
...
Nach der erfolgreiche Anmeldung erhält der Nutzer einen \acrfull{JWT}, um ihn bei weiteren Anfragen zu kodiert ( in \acrfull{JWS} ) präsentieren. Dies geschieht mithilfe von der \acrfull{XHR} Schnittstelle. In diesem Fall wurde deren Implementierung von jQuery benutzt.
...
Die Lambdafunktion mit \glqq UserProfile\grqq\ gekennzeichnet dekodiert den erhaltenen \acrshort{JWT} durch einen Schlüssel, der in Umgebungsvariablen ( siehe \autoref{lambda-env} ) angelegt wurde. Die Überprüfung erfolgt mit einer Abfrage an AuthO ( siehe \autoref{lst:verify} in Zeile 2 ).
...
Diese Authentisierungsvorgehen ( siehe \ref{pic:federated-identity} ) basiert auf das Vertrauen Zwischen zwei den Cloudanbietern. Der Nutzer kann für mehrere Anwendungen die gleiche Zugangsdaten benutzen und die Anmeldung erfolgt über den Identitätsanbieter ( wie z.B Google oder Github ). 
...
Die Partitionierung und das Replizieren von Datenbanken bringt zwar Vorteile ( wie in \autoref{data-vorteile} zitiert wurde ) aber auch Herausforderungen bei der Synchronisation und Konsistenz der Daten. Dies geschieht auf Grund des konsistent zu haltenden Zustandes der Datenbanken.

Die Relevanz und Bedeutung der Daten hängt von dem zu betrachtenden Sichtpunkt ab. Z.B ein Schüler braucht schnell zu erfahren ob der Hochladevorgang seiner Arbeit erfolgreich war ( Schreibzugriff vorziehen ). Ein Professor überprüft ob alle Schüler ihre Arbeit hochgeladen haben ( Lesezugriff vorziehen ). Daher lässt sich eine Technologisch orientierte Strategie unklar definieren, da weder alle Faktoren bekannt noch berücksichtigt werden können. 

Das Letztere widerspricht die Annahme, dass ein Service entweder eine Funktion oder eine Datenbank ist. 

Unter Service in \glqq \acrlong{SOA}\grqq\ kann als ein Mehrwert für die Benutzer interpretiert werden. 

Daraus ergibt sich, dass eine Funktionale Datenbankpartitionierung genauer die Benutzungsmuster der Anwendung entspricht und damit, wenn korrekt implementiert, von höher Performanz ist. 

Die \acrshort{EDA} eignet sich für die Steuerung von Benutzungsfällen besonders gut, da ein Event eine Interaktion zwischen dem Kunden und der Anwendung darstellen kann. 

Die Orchestrierung solcher Benutzungsfällen lässt sich mittels einer endlichen Zustandsmaschine definierten und einer Petrinetz darstellen, in der Events die Kanten und \acrshort{FaaS} die Ecken sind. Dieser Ansatz ähnelt dem Domain getriebenen Entwurf ( Domain Driven Design \cite{ddd} ), in dem Benutzungsfällen zusammen in Kontextgrenzen ( Bounded Context ) gefasst werden.

Die starke Entkopplung ( siehe \autoref{sec:eda} ) von Diensten erfordert allerdings ihren Zusammenhalt.
...
Frameworks helfen bei der Entwicklung von komplexen Systemen, in dem sie Abstraktionsschichten hinzufügen und damit komplexe Berechnungen wie bei der \glqq Dependency Injection \grqq\ von \acrfull{JEE}. 

Für \acrshort{KOMA} wurde kein Framework benutzt, da die Implementierung der Funktionen das \acrshort{SRP} gefolgt haben und die Abstraktionsschichten nicht bedurften. Hingegen wurden folgende Tests durchgeführt, um die Verwendbarkeit von Frameworks innerhalb einer Lambdafunktion auszuwerten. 

Das ausgewählte Framework ist Apache Jena \cite{fuseki} und als ein eingebetteter Server eingesetzt. Dieses unterstützt den Bau von Semantic Web und Linked Data Anwendugen.

Die \autoref{lst:fuseki} zeigt die vorgenommene Konfiguration für den Aufbau des Servers. Dieser wird während des Tests die schon vorgeladene \acrshort{KOMA} Ontologie durchgehe und ausloggen, um anschließend wieder zu stoppen.
...
% RES
Die folgende Graphik aufzeichnet die Ergebnisse der oben genannten Tests. 
...
Die Ergebnisse Zeigen auf einer Seite, wie sich die Lambdalaufzeitumgebung initiiert ( siehe \ref{lambda-cold-warm} ). Da die Lambdafunktion für jede Anfrage neu instantiiert, muss das Server neu gestartet werden. Hierzu beweist die Graphik, dass \acrshort{IaaS} und \acrshort{PaaS} besser für längerfristige Prozesse als Lambda geeignet sind ( siehe \ref{lambda-yes-no} ). 
Die bereitgestellte Speicherkapazität beschleunigt die Ausführung und dadurch verkürzt die Antwortzeiten. Diese Szenario nutzt die Fähigkeiten des eingebetteten Servers nicht, wie Transaktionsverwaltung oder \glqq In-Memory\grqq\ Caching. 

An dieser Stelle empfehlt sich das in \autoref{legacy-api-proxy} vorgestellte Legacy API Architekturmuster zu benutzen. Somit würde Anfragen die Transaktionsverwaltung bedürfen an den Server geführt und die Latenz verkürzt.
...
Frameworks helfen aber sind Plattform abhängig. Entweder \acrshort{JEE} und \acrshort{JVM} oder PHP.
